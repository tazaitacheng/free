...
#import "Masonry.h"
#import "HJDropDownMenuTitleButton.h"
#import "HJDropDownMenuCollectionViewCell.h"
#import "HJDropDownMenuUICalc.h"

typedef void (^HJDropDownMenuAnimateCompleteHandler)(void);
...
{
    HJIndexPath* indexPath = [[self alloc]initWithColumn:col row:row];
    return indexPath;
}

@end

@interface HJDropDownMenu ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@property(nonatomic,weak)HJDropDownMenuTitleButton* selectedButton;
@property(nonatomic,weak)HJDropDownMenuCollectionViewCell* defaultSelectedCell;

@end

static NSString* const collectionCellID = @"HJDropDownMenuCollectionViewCell";
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [_collectionView registerClass:[HJDropDownMenuCollectionViewCell class] forCellWithReuseIdentifier:collectionCellID];
...
- (void)setDataSource:(id<HJDropMenuDataSource>)dataSource
{
    ...
    HJDropDownMenuTitleButton *lastTitleButton = nil;
    for (NSInteger index = 0; index < _numOfMenu; index++) {
        
        NSString *titleString = [_dataSource menu:self titleForRowAtIndexPath:[HJIndexPath indexPathWithColumn:index row:0]];
        HJDropDownMenuTitleButton *titleButton = [[HJDropDownMenuTitleButton alloc] initWithMainTitle:[_dataSource menu:self titleForColumn:index] subTitle:titleString];
...
- (void)animationWithTitleButton:(HJDropDownMenuTitleButton *)button BackgroundView:(UIView *)backgroundView
                  collectionView:(UICollectionView *)collectionView
                            show:(BOOL)isShow
                        complete:(HJDropDownMenuAnimateCompleteHandler)complete
{
    WS(weakSelf);
...
- (void)titleButtonDidClick:(HJDropDownMenuTitleButton *)titleButton
{
    clickCount++;
    WS(weakSelf);
...
{
    WS(weakSelf);
    HJDropDownMenuTitleButton *titleButton = self.titleButtons[self.currentSelectedMenuIndex];
    [self animationWithTitleButton:titleButton BackgroundView:self.backgroundView collectionView:self.collectionView show:NO complete:^{
        weakSelf.show = NO;
    }];
}
...
{
    HJDropDownMenuCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:collectionCellID forIndexPath:indexPath];
    ...
    cell.contentString = [self.dataSource menu:self titleForRowAtIndexPath:[HJIndexPath indexPathWithColumn:self.currentSelectedMenuIndex row:indexPath.row]];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.delegate menu:self didSelectRowAtIndexPath:[HJIndexPath indexPathWithColumn:self.currentSelectedMenuIndex row:indexPath.row]];
...
{
    WS(weakSelf);
    HJDropDownMenuTitleButton *button = _titleButtons[self.currentSelectedMenuIndex];
    NSString *currentSelectedTitle = [self.dataSource menu:self titleForRowAtIndexPath:[HJIndexPath indexPathWithColumn:self.currentSelectedMenuIndex row:row]];
